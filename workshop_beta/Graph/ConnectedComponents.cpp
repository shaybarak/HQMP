...
void Connected_components::print_connected_components() {
	...
	std::cout <<"cc id\t"<<"|"<<"cc elements"<<std::endl;
	std::cout <<"--------"<<"|"<<"--------------------"<<std::endl;

	int multiple_elem_cc_size = 0;
	if (id_cc_map.size() > 20) {
		for (Int_cc_map::iterator iter = id_cc_map.begin(); iter != id_cc_map.end(); ++iter) {
			if (iter->second.size() > 1) {
				multiple_elem_cc_size++;
				print_connected_component(iter->first);
			}
		}
		std::cout << (id_cc_map.size() - multiple_elem_cc_size) << " single elements components : ";
		for (Int_cc_map::iterator iter = id_cc_map.begin(); iter != id_cc_map.end(); ++iter) {
			if (iter->second.size() == 1) {
...
void Connected_components::print_connected_component(int cc_id) {
	...
	if (iter->second.size() > 10) {
		std::cout << "contains " << iter->second.size() << " elements";
	} else {
